...
for model in models:
    for folder_name in os.listdir(dir_path):
        ...
        for i in range(1,4):
            ...
            api_answer = api_call(full_input_text, model)
            
            # Get the Python code from Ollama output
            api_python = get_python(api_answer)

            # Save the generated Python code to 'submissions_[model]/[problemid].py'
            ...
            text_output_file = f"{i}.txt"  # File name for the full Ollama response
            ...
            file_path = os.path.join(submissions_folder, output_file)
            ...
            result_path = os.path.join(results_folder, f"{i}.txt")
